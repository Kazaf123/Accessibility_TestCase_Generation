...
public class MapsLayersTest {
    private static final String TAG = "A11yTest";

    @Test
    public void testLayersMissingLabel() throws Exception {
        UiDevice device = UiDevice.getInstance(InstrumentationRegistry.getInstrumentation());
        UiObject2 targetNode = device.findObject(By.res("com.google.android.apps.maps:id/layers_button"));

        if (targetNode == null) {
            fail("Node with resource-id com.google.android.apps.maps:id/layers_button not found on screen.");
            return;
        }
        ...
        if (missingLabel) {
            try {
                File out = new File(Environment.getExternalStorageDirectory(), "maps_layers_nolabel.png");
