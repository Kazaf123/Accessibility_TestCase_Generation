...
public class WhatsAppVoiceButtonTest {
    private static final String TAG = "A11yTest";

    @Test
    public void testVoiceButtonMissingLabel() throws Exception {
        UiDevice device = UiDevice.getInstance(InstrumentationRegistry.getInstrumentation());
        UiObject2 targetNode = device.findObject(By.res("com.whatsapp:id/voice_note_btn"));

        if (targetNode == null) {
            fail("Node with resource-id com.whatsapp:id/voice_note_btn not found on screen.");
            return;
        }
        ...
        if (missingLabel) {
            try {
                File out = new File(Environment.getExternalStorageDirectory(), "wa_voice_nolabel.png");
