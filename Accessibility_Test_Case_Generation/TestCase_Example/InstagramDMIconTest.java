...
public class InstagramDMIconTest {
    private static final String TAG = "A11yTest";

    @Test
    public void testDMIconMissingLabel() throws Exception {
        UiDevice device = UiDevice.getInstance(InstrumentationRegistry.getInstrumentation());
        UiObject2 targetNode = device.findObject(By.res("com.instagram.android:id/action_bar_message_icon"));

        if (targetNode == null) {
            fail("Node with resource-id com.instagram.android:id/action_bar_message_icon not found on screen.");
            return;
        }
        ...
        if (missingLabel) {
            try {
                File out = new File(Environment.getExternalStorageDirectory(), "ins_dm_nolabel.png");
