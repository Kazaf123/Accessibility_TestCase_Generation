...
public class ZoomSwitchCameraTest {
    private static final String TAG = "A11yTest";

    @Test
    public void testSwitchCameraMissingLabel() throws Exception {
        UiDevice device = UiDevice.getInstance(InstrumentationRegistry.getInstrumentation());
        UiObject2 targetNode = device.findObject(By.res("us.zoom.videomeetings:id/btnSwitchCamera"));

        if (targetNode == null) {
            fail("Node with resource-id us.zoom.videomeetings:id/btnSwitchCamera not found on screen.");
            return;
        }
        ...
        if (missingLabel) {
            try {
                File out = new File(Environment.getExternalStorageDirectory(), "zoom_cam_nolabel.png");
