...
public class TwitterComposeTest {
    private static final String TAG = "A11yTest";

    @Test
    public void testComposeMissingLabel() throws Exception {
        UiDevice device = UiDevice.getInstance(InstrumentationRegistry.getInstrumentation());
        UiObject2 targetNode = device.findObject(By.res("com.twitter.android:id/composer_write"));

        if (targetNode == null) {
            fail("Node with resource-id com.twitter.android:id/composer_write not found on screen.");
            return;
        }
        ...
        if (missingLabel) {
            try {
                File out = new File(Environment.getExternalStorageDirectory(), "tw_compose_nolabel.png");
