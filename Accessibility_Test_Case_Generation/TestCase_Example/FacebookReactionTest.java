...
public class FacebookReactionTest {
    private static final String TAG = "A11yTest";

    @Test
    public void testReactionMissingLabel() throws Exception {
        UiDevice device = UiDevice.getInstance(InstrumentationRegistry.getInstrumentation());
        UiObject2 targetNode = device.findObject(By.res("com.facebook.katana:id/reaction_btn"));

        if (targetNode == null) {
            fail("Node with resource-id com.facebook.katana:id/reaction_btn not found on screen.");
            return;
        }
        ...
        if (missingLabel) {
            try {
                File out = new File(Environment.getExternalStorageDirectory(), "fb_reaction_nolabel.png");
