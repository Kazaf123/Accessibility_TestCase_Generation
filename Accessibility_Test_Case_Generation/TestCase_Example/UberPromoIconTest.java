...
public class UberPromoIconTest {
    private static final String TAG = "A11yTest";

    @Test
    public void testPromoIconMissingLabel() throws Exception {
        UiDevice device = UiDevice.getInstance(InstrumentationRegistry.getInstrumentation());
        UiObject2 targetNode = device.findObject(By.res("com.ubercab:id/promo_info_icon"));

        if (targetNode == null) {
            fail("Node with resource-id com.ubercab:id/promo_info_icon not found on screen.");
            return;
        }
        ...
        if (missingLabel) {
            try {
                File out = new File(Environment.getExternalStorageDirectory(), "uber_promo_nolabel.png");
