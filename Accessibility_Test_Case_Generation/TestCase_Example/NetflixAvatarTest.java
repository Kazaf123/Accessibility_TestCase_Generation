...
public class NetflixAvatarTest {
    private static final String TAG = "A11yTest";

    @Test
    public void testAvatarMissingLabel() throws Exception {
        UiDevice device = UiDevice.getInstance(InstrumentationRegistry.getInstrumentation());
        UiObject2 targetNode = device.findObject(By.res("com.netflix.mediaclient:id/profile_avatar"));

        if (targetNode == null) {
            fail("Node with resource-id com.netflix.mediaclient:id/profile_avatar not found on screen.");
            return;
        }
        ...
        if (missingLabel) {
            try {
                File out = new File(Environment.getExternalStorageDirectory(), "netflix_avatar_nolabel.png");
