...
public class GmailFilterTest {
    private static final String TAG = "A11yTest";

    @Test
    public void testFilterMissingLabel() throws Exception {
        UiDevice device = UiDevice.getInstance(InstrumentationRegistry.getInstrumentation());
        UiObject2 targetNode = device.findObject(By.res("com.google.android.gm:id/filter_icon"));

        if (targetNode == null) {
            fail("Node with resource-id com.google.android.gm:id/filter_icon not found on screen.");
            return;
        }
        ...
        if (missingLabel) {
            try {
                File out = new File(Environment.getExternalStorageDirectory(), "gmail_filter_nolabel.png");
