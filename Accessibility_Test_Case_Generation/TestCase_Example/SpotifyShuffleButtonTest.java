...
public class SpotifyShuffleButtonTest {
    private static final String TAG = "A11yTest";

    @Test
    public void testShuffleButtonMissingLabel() throws Exception {
        UiDevice device = UiDevice.getInstance(InstrumentationRegistry.getInstrumentation());
        UiObject2 targetNode = device.findObject(By.res("com.spotify.music:id/shuffle_button"));

        if (targetNode == null) {
            fail("Node with resource-id com.spotify.music:id/shuffle_button not found on screen.");
            return;
        }
        ...
        if (missingLabel) {
            try {
                File out = new File(Environment.getExternalStorageDirectory(), "spotify_shuffle_nolabel.png");
