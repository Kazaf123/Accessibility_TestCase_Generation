...
public class YouTubeCastIconTest {
    private static final String TAG = "A11yTest";

    @Test
    public void testCastIconMissingLabel() throws Exception {
        UiDevice device = UiDevice.getInstance(InstrumentationRegistry.getInstrumentation());
        UiObject2 targetNode = device.findObject(By.res("com.google.android.youtube:id/media_route_button"));

        if (targetNode == null) {
            fail("Node with resource-id com.google.android.youtube:id/media_route_button not found on screen.");
            return;
        }
        ...
        if (missingLabel) {
            try {
                File out = new File(Environment.getExternalStorageDirectory(), "yt_cast_nolabel.png");
