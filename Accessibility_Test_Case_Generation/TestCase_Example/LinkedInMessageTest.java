...
public class LinkedInMessageTest {
    private static final String TAG = "A11yTest";

    @Test
    public void testMessageMissingLabel() throws Exception {
        UiDevice device = UiDevice.getInstance(InstrumentationRegistry.getInstrumentation());
        UiObject2 targetNode = device.findObject(By.res("com.linkedin.android:id/message_icon"));

        if (targetNode == null) {
            fail("Node with resource-id com.linkedin.android:id/message_icon not found on screen.");
            return;
        }
        ...
        if (missingLabel) {
            try {
                File out = new File(Environment.getExternalStorageDirectory(), "linkedin_msg_nolabel.png");
