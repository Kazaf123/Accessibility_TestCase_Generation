...
public class ChromeNewTabTest {
    private static final String TAG = "A11yTest";

    @Test
    public void testNewTabMissingLabel() throws Exception {
        UiDevice device = UiDevice.getInstance(InstrumentationRegistry.getInstrumentation());
        UiObject2 targetNode = device.findObject(By.res("com.android.chrome:id/new_tab_button"));

        if (targetNode == null) {
            fail("Node with resource-id com.android.chrome:id/new_tab_button not found on screen.");
            return;
        }
        ...
        if (missingLabel) {
            try {
                File out = new File(Environment.getExternalStorageDirectory(), "chrome_newtab_nolabel.png");
