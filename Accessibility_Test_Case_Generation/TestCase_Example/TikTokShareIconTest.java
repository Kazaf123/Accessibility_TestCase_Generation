...
public class TikTokShareIconTest {
    private static final String TAG = "A11yTest";

    @Test
    public void testShareIconMissingLabel() throws Exception {
        UiDevice device = UiDevice.getInstance(InstrumentationRegistry.getInstrumentation());
        UiObject2 targetNode = device.findObject(By.res("com.zhiliaoapp.musically:id/share_icon"));

        if (targetNode == null) {
            fail("Node with resource-id com.zhiliaoapp.musically:id/share_icon not found on screen.");
            return;
        }
        ...
        if (missingLabel) {
            try {
                File out = new File(Environment.getExternalStorageDirectory(), "tiktok_share_nolabel.png");
