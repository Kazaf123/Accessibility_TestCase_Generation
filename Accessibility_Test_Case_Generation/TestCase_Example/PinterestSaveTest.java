...
public class PinterestSaveTest {
    private static final String TAG = "A11yTest";

    @Test
    public void testSaveMissingLabel() throws Exception {
        UiDevice device = UiDevice.getInstance(InstrumentationRegistry.getInstrumentation());
        UiObject2 targetNode = device.findObject(By.res("com.pinterest:id/save_pin_button"));

        if (targetNode == null) {
            fail("Node with resource-id com.pinterest:id/save_pin_button not found on screen.");
            return;
        }
        ...
        if (missingLabel) {
            try {
                File out = new File(Environment.getExternalStorageDirectory(), "pin_save_nolabel.png");
